...
class _WrkHeader:
    ...
    sysex_bank: int | None = None
    sysex_length: int | None = None
    sysex_autosend: bool | None = None
    sysex_name: str | None = None
    ...
    new_sysex_bank: int | None = None
    new_sysex_length: int | None = None
...
def _read_midi_file_bytes(data: bytes, path: Path) -> MidiFile:
    if _looks_like_wrk(data, path):
        ...
        if header.meter_count is not None:
            details += f", meter entries {header.meter_count}"
            if header.plain_meter_measure is not None:
                details += (
        ...
        if header.sysex_bank is not None:
            details += (
                f", classic sysex bank {header.sysex_bank}"
                f" bytes {header.sysex_length}"
            )
            if header.sysex_autosend is not None:
                details += f" autosend {int(header.sysex_autosend)}"
            if header.sysex_name:
                details += f" '{header.sysex_name}'"
        ...
        if header.new_sysex_bank is not None:
            details += (
                f", new sysex bank {header.new_sysex_bank}"
                f" bytes {header.new_sysex_length}"
                f" port {header.new_sysex_port}"
            )
            if header.new_sysex_autosend is not None:
...
def _read_wrk_header(data: bytes, path: Path) -> _WrkHeader:
    ...
    if first_chunk_id == 1:
    ...
    elif first_chunk_id == 5:
        if first_chunk_length < 2:
            raise MidiFileError("Corrupted Cakewalk WRK file")
        meter_count = reader.read_u16_le()
        if meter_count > 0:
            if reader.remaining < 12:
                raise MidiFileError("Corrupted Cakewalk WRK file")
            reader.read(4)
            plain_meter_measure = reader.read_u16_le()
        ...
        if new_tempo_count > 0:
            ...
            reader.read(8)
    elif first_chunk_id == 6:
        if first_chunk_length < 5:
            raise MidiFileError("Corrupted Cakewalk WRK file")
        sysex_bank = reader.read(1)[0]
        sysex_length = reader.read_u16_le()
        sysex_autosend = reader.read(1)[0] != 0
        name_length = reader.read(1)[0]
        if reader.remaining < name_length + sysex_length:
            raise MidiFileError("Corrupted Cakewalk WRK file")
        sysex_name = reader.read(name_length).decode("latin-1", errors="replace")
        reader.read(sysex_length)
    ...
    elif first_chunk_id == 44:
        if first_chunk_length < 10:
            raise MidiFileError("Corrupted Cakewalk WRK file")
        new_sysex_bank = reader.read_u16_le()
        new_sysex_length = reader.read_u32_le()
        new_sysex_port = reader.read_u16_le()
        new_sysex_autosend = reader.read(1)[0] != 0
        name_length = reader.read(1)[0]
        if reader.remaining < name_length + new_sysex_length:
            raise MidiFileError("Corrupted Cakewalk WRK file")
        new_sysex_name = reader.read(name_length).decode("latin-1", errors="replace")
        reader.read(new_sysex_length)
    return _WrkHeader(
        major_version=major_version,
        minor_version=minor_version,
        first_chunk_id=first_chunk_id,
        ...
        sysex_bank=sysex_bank,
        sysex_length=sysex_length,
        sysex_autosend=sysex_autosend,
        sysex_name=sysex_name,
        ...
        new_sysex_bank=new_sysex_bank,
        new_sysex_length=new_sysex_length,
