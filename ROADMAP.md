...
- [x] Allow manual note playing with computer keyboard and mouse where appropriate.
- [ ] Window menu:
  - [x] fullscreen;
...
- [x] Highlight past/future lyrics with configurable colors.
...
- [x] Create, modify, sort, open, and save playlists.
  - [x] Playlist rows now show friendly song titles while preserving the real file path internally.
...
- [x] Add WRK parser skeleton with explicit unsupported-feature errors.
  ...
  - [x] Parse `TRKOFFS_CHUNK` when it is the first WRK record and include the detected track offset in the unsupported message.
  - [x] Parse `TRKREPS_CHUNK` when it is the first WRK record and include the detected track repetitions in the unsupported message.
  - [x] Parse `TIMEFMT_CHUNK` when it is the first WRK record and include the detected time format and offset in the unsupported message.
...
- [x] Add user-selectable lyrics/text encoding to the sequence model.
